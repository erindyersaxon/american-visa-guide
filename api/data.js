export default async function handler(req, res) {
  ...
  // clusters is oldest-first; reverse at end for newest-first API response
  const clusterDates = clusters.map(c =>
    new Date(Math.min(...c.members.map(r => new Date(r.interview_letter)))).toISOString().split('T')[0]
  )

  // Observed UK drop times — community-sourced, used where member data gives 00:00
  ...
  // Include all clusters with at least 1 member — threshold of 3 excluded new
  // drops that haven't yet accumulated multiple submissions.
  ...
  // Derived entirely from live clusters — no hardcoded dates.
  ...
  return res.status(200).json({
    ...
    trends: {
      ...
      dq_to_interview: {
        ...
        last_6m:  { avg: trendDqToInterview(6),     median: trendDqToInterviewMe
  const median = (arr) => {
    const valid = arr.filter(n => n !== null && n > 0).sort((a, b) => a - b)
    if (!valid.length) return null
    const mid = Math.floor(valid.length / 2)
