export default async function handler(req, res) {
  ...
  const ilDropsAsc = clusters.map((cluster, i) => {
    ...
    // UK time: UTC+1 (BST, April-October) or UTC+0 (GMT, otherwise)
    const month = earliestIL.getUTCMonth() // 0-indexed
    const isBST = month >= 3 && month <= 9 // April-October
    const ukHours = (earliestIL.getUTCHours() + (isBST ? 1 : 0)) % 24
    const ukMins = earliestIL.getUTCMinutes()
    const time = String(ukHours).padStart(2,'0') + ':' + String(ukMins).padStart(2,'0')
  ...
  const ilDrops = ilDropsAsc.slice().reverse() // newest first for API response
