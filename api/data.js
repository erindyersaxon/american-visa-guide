export default async function handler(req, res) {
  ...
  // Helper: avg with outlier cap per field
  ...
  const stageAvgs = {
    ...
    approval_to_dos:     stageAvg(deduped, 'i130_approval', 'sent_to_dos', 90),
    dos_to_nvc_fees:     stageAvg(deduped, 'sent_to_dos', 'nvc_fees_paid', 180),
    ...
    approval_to_dq:      stageAvg(deduped, 'i130_approval', 'dq_date', 365),
  ...
  const observedDrops = [
    { date: '2026-04-17', time: '16:02' },
    { date: '2026-03-31', time: '15:16' },
    { date: '2026-03-04', time: '14:08' },
    { date: '2026-02-13', time: '13:51' },
    { date: '2026-01-15', time: '13:53' },
    { date: '2026-01-01', time: '13:59' },
    { date: '2025-12-11', time: '13:55' },
    { date: '2025-11-26', time: '15:03' },
    { date: '2025-11-11', time: '14:02' },
    { date: '2025-10-10', time: '15:03' },
    { date: '2025-09-11', time: '15:38' },
    { date: '2025-09-10', time: '15:03' },
    { date: '2025-08-11', time: '15:09' },
    { date: '2025-07-21', time: '15:00' },
    { date: '2025-06-06', time: '15:31' },
  ]

  const uniqueILDates = observedDrops.map(d => d.date) // already sorted desc

  // IL drop gaps + DQ range covered by each drop
  const ilDrops = observedDrops.map((drop, i) => {
    const date = drop.date
    const prev = uniqueILDates[i + 1]
    const gap = prev ? daysBetween(prev, date) : null

    // Find members whose IL arrived on this drop date (±1 day tolerance)
    const dropD = new Date(date + 'T12:00:00Z')
    const membersInDrop = standard.filter(r => {
      if (!r.interview_letter || !r.dq_date) return false
      const ilD = new Date(r.interview_letter)
      return Math.abs(ilD - dropD) <= 86400000 * 1.5 // within 1.5 days
    })

    // Get DQ date range covered by this drop
    const dqDates = membersInDrop
      .map(r => r.dq_date)
      .filter(Boolean)
      .sort()
    ...
    const il_count = membersInDrop.length

    return { date, time: drop.time, gap, dq_from, dq_to, dq_days, il_count }
  })
