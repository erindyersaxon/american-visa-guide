...
(async () => {
  try {
    ...
    // Outcomes
    const appPct      = out.total > 0 ? Math.round(((out.approved + (out.cleared||0)) / out.total) * 100) : 0
    const notAppPct   = out.total > 0 ? Math.round((out.not_approved / out.total) * 100) : 0
    const clearedPct  = out.total > 0 ? Math.round(((out.cleared||0) / out.total) * 100) : 0
    const pausePct    = out.total > 0 ? Math.round(((out.visa_pause||0) / out.total) * 100) : 0
    const deniedPct   = out.total > 0 ? Math.round(((out.denied||0) / out.total) * 100) : 0
    ...
    set('outcome-approved-n',   `${plural(out.approved, 'approved')}${out.cleared ? ' · ' + out.cleared + ' cleared 221g' : ''}`)
