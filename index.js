...
(async () => {
  try {
    ...
    const pipe_dq_il = d.trends?.dq_to_il?.last_6m ?? d.trends?.dq_to_il?.all_time
    if (pipe_dq_il) {
      const el = document.getElementById('pipe-dq-il')
      if (el) el.textContent = pipe_dq_il + 'd'
    }
    const lastDrop = d.observedDrops?.[0]
    if (lastDrop) {
      const el = document.getElementById('pipe-last-drop')
      if (el) el.textContent = typeof lastDrop === 'object' ? lastDrop.date : lastDrop
    }
    const nvcFees = sa?.approval_to_dos != null && sa?.dos_to_nvc_fees != null
      ? sa.approval_to_dos + sa.dos_to_nvc_fees
      : (sa?.approval_to_dos ?? null)
    if (nvcFees) {
      const el = document.getElementById('pipe-nvc-fees')
      if (el) el.textContent = Math.round(nvcFees) + 'd'
    }
    const dosToDq = (sa?.dos_to_nvc_fees ?? 0) + (sa?.fees_to_docs ?? 0) + (sa?.docs_to_dq ?? 0)
    const nvcDQ = sa?.approval_to_dos != null && dosToDq > 0
      ? sa.approval_to_dos + dosToDq : null
    if (nvcDQ) {
      const el = document.getElementById('pipe-nvc-dq')
      if (el) el.textContent = Math.round(nvcDQ) + 'd'
    }
    ...
    setBar('outcome-cleared-bar',  clearedPct)
    ...
    set('outcome-cleared-pct',  clearedPct + '%')
    set('outcome-cleared-n',    plural(out.cleared||0, 'case'))
    ...
    const funnelData = [
      ...
      ['funnel-dq',       sc.dq,                 sa.docs_to_dq != null ? `~${sa.docs_to_dq}d after docs` : null],
      ['funnel-il',       sc.interview_letter,   avg_dq_to_il      != null ? `~${avg_dq_to_il}d after DQ`      : null],
