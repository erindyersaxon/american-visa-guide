...
(async () => {
  try {
    ...
    const nvcDQ = sa?.approval_to_dq
    if (nvcDQ) {
      const el = document.getElementById('pipe-nvc-dq')
      if (el) el.textContent = Math.round(nvcDQ) + 'd'
    }
    const nvcStats = sa?.approval_to_dq_stats
    if (nvcStats) {
      const s = id => document.getElementById(id)
      if (s('nvc-dq-avg'))    s('nvc-dq-avg').textContent    = nvcStats.avg + 'd'
      if (s('nvc-dq-median')) s('nvc-dq-median').textContent = nvcStats.median + 'd'
      if (s('nvc-dq-range'))  s('nvc-dq-range').textContent  = nvcStats.min + '–' + nvcStats.max + 'd'
      if (s('nvc-dq-n'))      s('nvc-dq-n').textContent      = nvcStats.n + ' cases'
    }
