...
(async () => {
  try {
    ...
    // Update plain-english DQ→IL figure
    const plainDqIl = document.getElementById('plain-dq-il')
    if (plainDqIl && d.trends?.dq_to_il?.last_6m) plainDqIl.textContent = d.trends.dq_to_il.last_6m
    ...
    if (grid && il.recent_drops) {
      grid.innerHTML = il.recent_drops.map(drop => {
        ...
        return `<div class="drop-chip">
          ...
          <div class="drop-chip-gap">${drop.gap != null ? drop.gap + 'd gap' : 'first recorded'}${drop.time ? ' · ' + drop.time + ' UK' : ''}</div>
          ${dqSpan}${countSpan}
        </div>`
      }).join('')
    }
