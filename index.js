...
(async () => {
  try {
    ...
    funnelData.forEach(([id, val, dayLabel], i) => {
      // Duration is now the headline — populate funnel-days-i
      if (dayLabel) {
        const dl = document.getElementById(`funnel-days-${i}`)
        // Strip the "~Xd after Y" → just "~Xd"
        const shortLabel = dayLabel.replace(/ ?(after|from) .+$/, '').trim()
        if (dl) dl.textContent = shortLabel
      }
      // Member count is the footnote — populate id-n
      const nEl = document.getElementById(id + '-n')
      if (nEl && val != null) nEl.textContent = val + ' members'
      // Keep bar for any remaining bar elements
