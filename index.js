...
(async () => {
  try {
    ...
    // Stage timing cards
    const fmt = v => v != null ? `~${v}d` : null
    const stageCards = [
      ['s-approval-welcome', fmt(sa.approval_to_welcome), sc.nvc_welcome_letter],
      ['s-welcome-fees',     fmt(sa.welcome_to_fees),     sc.nvc_fees_paid],
      ['s-fees-docs',        fmt(sa.fees_to_docs),        sc.nvc_docs_submitted],
      ['s-docs-dq',          fmt(sa.docs_to_dq),          sc.dq],
