...
(async () => {
  try {
    ...
    const m  = d.meta
    const k  = d.key_stats
    const t6 = d.trends  // Use last_6m where available, fall back to all_time
    // Override key stats with 6-month figures for display
    const avg_dq_to_il       = t6?.dq_to_il?.last_6m       ?? k.avg_dq_to_il
    const avg_il_to_interview = t6?.il_to_interview?.last_6m ?? k.avg_il_to_interview
    const avg_dq_to_interview = t6?.dq_to_interview?.last_6m ?? k.avg_dq_to_interview
    const tr = d.trends
    const out= d.outcomes
    const sc = d.stage_counts
    const sa = d.stage_avgs
    console.log('stage_counts:', sc)
    const il = d.il_schedule

    // Pipeline status cards
    // pipe-dq-il and pipe-last-drop are not rendered (queue tiles show this data directly)
    // NVC stat elements removed — stage 2 now uses fixed text
    ...
    // Key stats
    set('stat-dq-il',      fmt(avg_dq_to_il))
    set('stat-il-int',     fmt(avg_il_to_interview))
    set('stat-dq-int',     fmt(avg_dq_to_interview))

    // Update stat card period labels
    const dqIlPeriod   = t6?.dq_to_il?.last_6m       != null ? 'Last 6 months' : 'All time'
    const ilIntPeriod  = t6?.il_to_interview?.last_6m != null ? 'Last 6 months' : 'All time'
    const dqIntPeriod  = t6?.dq_to_interview?.last_6m != null ? 'Last 6 months' : 'All time'
    set('stat-dq-il-period',  dqIlPeriod)
    ...
    set('trend-dq-il-dir',  trendDir(tr.dq_to_il.all_time, tr.dq_to_il.last_6m))
    ...
    set('trend-il-int-dir', trendDir(tr.il_to_interview.all_time, tr.il_to_interview.last_6m))
    ...
    set('trend-dq-int-dir', trendDir(tr.dq_to_interview.all_time, tr.dq_to_interview.last_6m))

    // Outcomes
    const total      = out.total || 1
    const appPct     = Math.round(((out.approved + (out.cleared||0)) / total) * 100)
    const notAppPct  = Math.round((out.not_approved / total) * 100)
    const clearedPct = Math.round(((out.cleared||0) / total) * 100)
    const pausePct   = Math.round(((out.visa_pause||0) / total) * 100)
    const deniedPct  = Math.round(((out.denied||0) / total) * 100)
    const setBar = (id, pct) => { const el = document.getElementById(id); if (el) el.style.width = pct + '%' }
    setBar('outcome-approved-bar', appPct)
    setBar('outcome-notapp-bar',   notAppPct)
    setBar('outcome-pause-bar',    pausePct)
    setBar('outcome-denied-bar',   deniedPct)
    const plural = (n, word) => `${n} ${word}${n === 1 ? '' : 's'}`
    set('outcome-approved-pct', appPct + '%')
    set('outcome-approved-n',   `${out.approved + (out.cleared||0)} approved${out.cleared ? ' (' + out.cleared + ' cleared 221g)' : ''}`)
    set('outcome-notapp-pct',   notAppPct + '%')
    set('outcome-notapp-n',     plural(out.not_approved, 'case'))
    set('outcome-pause-pct',    pausePct + '%')
    set('outcome-pause-n',      plural(out.visa_pause||0, 'case'))
    set('outcome-denied-pct',   deniedPct + '%')
    set('outcome-denied-n',     plural(out.denied||0, 'case'))
    setText('outcome-total', out.total)

    // Stage timing cards
    const fmtAvg = v => v != null ? `~${v}d` : null
    const stageCards = [
      ['s-approval-welcome', fmtAvg(sa.approval_to_welcome), sc.nvc_welcome_letter],
      ['s-welcome-fees',     fmtAvg(sa.welcome_to_fees),     sc.nvc_fees_paid],
      ['s-fees-docs',        fmtAvg(sa.fees_to_docs),        sc.nvc_docs_submitted],
      ['s-docs-dq',          fmtAvg(sa.docs_to_dq),          sc.dq],
      ['s-dq-il',            avg_dq_to_il != null ? `~${avg_dq_to_il}d` : null, sc.interview_letter],
      ['s-il-medical',       fmt(sa.il_to_medical),       sc.medical],
      ['s-medical-interview', null,                       sc.interview],
      ['s-interview-passport', fmt(sa.interview_to_passport), sc.passport_in_hand],
    ]
    stageCards.forEach(([id, dur, members]) => {
      const durEl = document.getElementById(id)
      if (durEl && dur) durEl.innerHTML = dur
      const nEl = document.getElementById(id + '-n')
      if (nEl && members != null) nEl.textContent = members + ' members'
    })
    ...
    if (grid && il.recent_drops) {
      grid.innerHTML = il.recent_drops.map((drop, idx) => {
        ...
        const chipStyle = isLatest
          ...
          : ''
        const dateStyle = isLatest
          ? ` style="font-size:14px;color:var(--teal-light);"`
          : ''
        const dateRow = isLatest
          ? `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;"><div class="drop-chip-date"${dateStyle}>${fmtDate(drop.date)}</div>${latestBadge}</div>`
    ...
    // and no new drop has occurred since the last known drop
    const overdueEl = document.getElementById('drop-overdue-notice')
    if (overdueEl && il.estimated_next_window && il.last_il_drop) {
      try {
        // Parse end of window — format is "DD Mon YYYY – DD Mon YYYY"
        const windowStr = il.estimated_next_window
        const endPart = windowStr.includes('–') ? windowStr.split('–')[1].trim() : windowStr.trim()
        // Convert "20 May 2026" to a date
        const windowEnd = new Date(endPart + 'T23:59:59Z')
        const today = new Date()
        const lastDrop = new Date(il.last_il_drop + 'T00:00:00Z')
        // Show if today is past window end AND last drop hasn't changed (no new drop)
        // We check if the gap since last drop exceeds avg_gap + 7 days buffer
        const daysSinceLastDrop = Math.floor((today - lastDrop) / 86400000)
        const avgGap = il.avg_gap_days || 23
        if (today > windowEnd && daysSinceLastDrop > avgGap + 5) {
    ...
    if (week) {
      ...
      document.getElementById('week-dq').innerHTML         = renderMembers(week.dqs,         'None recorded')
      document.getElementById('week-approved').innerHTML   = renderMembers(week.approved,    'None recorded')
    }
